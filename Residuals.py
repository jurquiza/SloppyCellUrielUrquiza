...
class PeriodCheckfftResidual(Residual):
    ...
    def GetRequiredVarsByCalc(self):
        # Need the points between xVal and xVal + 2*periods
        magnitude = (self.xVal+2.0*self.yMeas-self.xVal,self.xVal)*20
        dense = list(numpy.linspace(self.xVal,self.xVal+2.0*self.yMeas,magnitude))
        return {self.cKey: {self.yKey: dense}}
